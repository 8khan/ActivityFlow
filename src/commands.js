...
function registerCommands(context, updateStatusBar, updateTimerDisplay) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('activity-tracker.startTimer', () => {
   ...
   */
  context.subscriptions.push(
    vscode.commands.registerCommand('activity-tracker.pauseTimer', () => {
      if (isRunning && !isPaused) {
        timer.pause();
        isPaused = true;
        ...
        vscode.window.showWarningMessage(
          'Timer is not running or already paused.',
        );
      }
    }),
  );
   ...
   */
  context.subscriptions.push(
    vscode.commands.registerCommand('activity-tracker.stopTimer', async () => {
      if (isRunning) {
        timer.stop();
        isRunning = false;
        isPaused = false;
   ...
   */
  context.subscriptions.push(
    vscode.commands.registerCommand('activity-tracker.viewLogs', async () => {
      try {
...
module.exports = { registerCommands, isRunning, isPaused };
